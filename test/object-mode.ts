...
test('data send/receive Buffer {objectMode: true}', function (t) {
  ...
  function tryTest () {
    ...
    peer2.on('data', function (data) {
      ...
      t.deepEqual(Buffer.from(data as Uint8Array), Buffer.from('this is a Buffer'), 'got correct message')

      peer2.send(Buffer.from('this is another Buffer'))
      peer1.on('data', function (data) {
        t.ok(ArrayBuffer.isView(data), 'data is a Buffer')
        t.deepEqual(Buffer.from(data as Uint8Array), Buffer.from('this is another Buffer'), 'got correct message')
...
test('data send/receive Uint8Array {objectMode: true}', function (t) {
  ...
  function tryTest () {
    ...
    peer2.on('data', function (data) {
      ...
      t.ok(ArrayBuffer.isView(data), 'data is a Buffer')
      t.deepEqual(Buffer.from(data as Uint8Array), Buffer.from([0, 1, 2]), 'got correct message')

      peer2.send(new Uint8Array([1, 2, 3]))
      peer1.on('data', function (data) {
        t.ok(ArrayBuffer.isView(data), 'data is a Buffer')
        t.deepEqual(Buffer.from(data as Uint8Array), Buffer.from([1, 2, 3]), 'got correct message')
