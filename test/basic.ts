...
test('data send/receive text', function (t) {
  ...
  function tryTest () {
    ...
    peer2.on('data', function (data) {
      ...
      t.equal(Buffer.from(data as Uint8Array).toString(), 'sup peer2', 'got correct message')

      peer2.send('sup peer1')
      peer1.on('data', function (data) {
        t.ok(ArrayBuffer.isView(data), 'data is Buffer')
        t.equal(Buffer.from(data as Uint8Array).toString(), 'sup peer1', 'got correct message')
